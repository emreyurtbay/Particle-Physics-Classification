---
title: "Analysis 2"
author: "Emre Yurtbay, Shashank Mahesh"
date: "12/16/2019"
output:
...
data1 <- readr::read_csv("https://www.dropbox.com/s/34gwx6e5mwkts2u/DATA1.csv?dl=1")
```
...
## Analysis I
```{r}
# Create a validation set and a training set from data1
set.seed(1)

#Find the sample size, which is 505 of data
smp_siz <- floor(0.8*nrow(data1))  

#Randomly finds rows equal to sample size and indices of those rows
data1_ind <- sample(seq_len(nrow(data1)),size = smp_siz)  

#One dataset includes these randomly found rows, the other does not
data1_train <- data1[data1_ind,] 
data1_val <- data1[-data1_ind,]  
```

```{r}
# Break up data into labels and predictors
train_labels <- data1_train$Label
train_predictors <- data1_train %>% dplyr::select(-c("Label"))
val_predictors <- data1_val %>% dplyr::select(-c("Label"))
val_labels <- data1_val$Label

# Filter out ghost particles and remove the label, as it should not be used as a predictor
no_ghost_train <- data1_train %>% filter(Label != 'Ghost')
...
calc_qda_vals <- function(data1, tau_val, select_tau = FALSE) {
...
```{r}
# Learn QDA model
out_qda <- calc_qda_vals(data1 = ghost_train_df, tau_val = 0.5, select_tau = TRUE)
```
...
calc_lda_vals <- function(data1, select_tau = FALSE) {
  set.seed(1)
...
```{r}
# Fit LDA models
out_lda <- calc_lda_vals(data1 = ghost_train_df, select_tau = TRUE)
```
...
```{r}
# Filter ghosts
noGhost <- data1 %>% dplyr::filter(Label != "Ghost")
...
#Randomly finds rows equal to sample size and indices of those rows
data1_ind <- sample(seq_len(nrow(noGhost)),size = smp_siz)  

#One dataset includes these randomly found rows, the other does not
train <- noGhost[data1_ind,] 
val <- noGhost[-data1_ind,]  
```
...
data1 <- data1 %>% dplyr::select(-c(X1))

data1 %>% mutate(
  ghostLabel = ifelse(Label == "Ghost", 1, 0)
) -> data1_withGhostColumn

data1_withGhostColumn <- data1_withGhostColumn %>% dplyr::select(-c("Label"))
```
...
dec_tree_model <- tree::tree(ghostLabel ~ ., data = data1_withGhostColumn )
pruned_model <- tree::prune.tree(dec_tree_model, best = 6)
```
